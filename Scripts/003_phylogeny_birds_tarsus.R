...
# This script is to build the phylogeny for birds for the dataset where body size measures are tarsus length instead of wing length
# for species whose two measures are available. The script estimates the 
...
birddata<-read.csv("Data/birddata_def_tarsus.csv", header = TRUE, stringsAsFactors = FALSE) #use birddatadef2 for tarsus
...
tiff("Results/Birds/Phylogeny/bird_phylogenetic_tree_pruned_tarsus.tiff",
     height=80, width=10,
...
save(tree_random, file = "Data/Final data/bird_tree_random_tarsus.Rdata")
...
# save the new tree
write.tree(bird.tree_random.fixed, file = "Data/Final data/bird.tree_random.fixed_tarsus.tre")
...
save(bird_phylo_cor, file = "Data/Final data/bird_phylo_cor_tarsus.Rdata")

# exporting fixed dataset for analyses
write.csv(birddata_ph, 
           "Data/Final data/birddata_ph_tarsus.csv", row.names = FALSE)

# saving session information with all packages versions for reproducibility purposes
sink("Data/Final data/bird_phylogeny_tarsus_R_session.txt")
sessionInfo()
sink()
